# 5. app_entrenar_modelo.py
# Aplicación principal para entrenar
...
def main():
    ...
    # 3. Crear datasets
    train_ds = procesador.crear_dataset(datos['train'][0], train_enc)
    valid_ds = procesador.crear_dataset(datos['valid'][0], valid_enc)
    test_ds = procesador.crear_dataset(datos['test'][0], test_enc)
    
    # 4. Crear y compilar modelo
    modelo = ModeloCNN(TAMAÑO_IMAGEN, procesador.num_clases)
    modelo.compilar()
    modelo.resumen()
    
    # 5. Entrenar
    historial = modelo.entrenar(train_ds, valid_ds, EPOCHS)
    ...
    # 8. Guardar modelo
    modelo.guardar('modelo_marcadores.h5')
